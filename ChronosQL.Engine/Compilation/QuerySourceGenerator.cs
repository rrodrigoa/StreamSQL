...
internal sealed class QuerySourceGenerator
{
    ...
    public string Generate()
    {
        ...
        if (_plan.WhereCondition is not null)
        {
            builder.AppendLine("            .Where(streamEvent => !streamEvent.IsData || MatchesWhere(streamEvent.Payload))");
        }
        ...
        if (_plan.WhereCondition is not null)
        {
            ...
            builder.AppendLine("    private static bool MatchesWhere(JsonElement payload)");
            builder.AppendLine("    {");
    ...
    {
        var methodName = $"EvaluatePredicate_{_predicateIndex++}";
    ...
    private string GetExpressionAccessorName(SqlExpression expression)
    {
        ...
        {
            name = $"TryGetExpression_{_expressionIndex++}";
